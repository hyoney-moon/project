	...
	<div class="container-sm w-25">
		<form action="hostJoin" method="post" enctype="multipart/form-data"
			...
			<input name="nickName" id="nickName"
				...
				title="닉네임은 문자 2자 이상입니다."> <input name="email" id="email"
...
<script>
	...
	$(function(){
		...
		let id = $("#hostId").val();	
		if(!id){
			$("#hostId").html("아이디를 입력하세요");
			return false;
		}
		$.ajax({url:"/host/check_id",
		 	data:"id="+id,
			dataType:"text"}
		).done(function(data){
			if(data == ""){
				$("#hostIdDiv").append("사용할 수 있는 아이디 입니다.")
			}else{
				$("#hostIdDiv").append("이미 사용중인 아이디 입니다.")
			}
		})
	});
	})
	...
	function confirmId() {
		...
		// hostId 값을 가지고 중복 검사를 위한 ajax 요청
		$
				.ajax({
					url : addr,
					data : {
	...
	function check() {
		...
		if (!pattern1.test(password) || !pattern2.test(password)
				|| !pattern3.test(password) || password.length < 8) {
