...
/*	@Transactional
	...
	List<ReviewDto> selectReview(@Param("boardNum")int boardNum);
	
	//리뷰 조회 (추천순)
	@Query(value="select * from review where boardNum = :boardNum order by likes_count desc", nativeQuery=true )
	List<ReviewDto> selectReviewOrderbyLikes(@Param("boardNum")int boardNum);
	
	
	//리뷰 조회 (내가 쓴 글 보기)
	@Query(value="select * from review where boardNum = :boardNum and cust_id = :cust_id order by review_asc desc", nativeQuery=true )
	List<ReviewDto> selectReviewOrderbyUserId(@Param("boardNum")int boardNum, @Param("cust_id")String cust_id);
	...
	//리뷰 추가
	@Query(value="insert into review (boardNum, cust_id, review_title, review_content, review_viewCount, review_star, review_isPrivate, review_asc, review_answer, review_date, review_img, likes_count) values(:boardNum, :cust_id, :review_title, :review_content, 0, :review_star, :review_isPrivate ,:review_asc, 0, now(), :review_img, 0)", nativeQuery=true )
	void insertReview(@Param("boardNum")int boardNum, @Param("cust_id")int cust_id, 
			...
			@Param("review_star")float review_star, @Param("review_isPrivate")int review_isPrivate, 
	...
	List<ReviewDto> MaxReview_asc(@Param("boardNum")int boardNum);
